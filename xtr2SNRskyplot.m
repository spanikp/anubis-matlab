...
% Default options
opt = struct('colorBarLimits',[15,55],...
             'colorBarTicks', 15:5:55,...
             'colorBarOn',false,...
             'figResolution','200',...
...
for i = 1:length(GNScell)
    ...
    if options.colorBarOn
        ...
        ylabel(c,sprintf('%s RMS MP%s value (cm)',GNScell{i},SNRcode),'fontsize',10,'fontname','arial')
    else
        caxis(options.colorBarLimits)
    end
    ...
    if saveFig == true
       ...
       figName = [fullfile(options.figSavePath, splittedInputName{end-1}), '_', GNScell{i}, '_SNR', SNRcode];
